...
void main () 
{
  if(is_texture_present == 1){
  	...
  	mat3 good_matrix = inverse(mat3(view_matrix));
  	vec3 ori_position = good_matrix*(vec3(position_pt));
  	if(ori_position.y < 2.0 && ori_position.x < 1.0 && ori_position.x > -1.0 && ori_position.y > 0.0)
